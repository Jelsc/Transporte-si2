...
class DispositivoFCMSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'token_fcm', 'tipo_dispositivo', 'nombre_dispositivo', 'activo', 'fecha_registro', 'fecha_ultimo_uso']
    ...
    def create(self, validated_data):
        ...
        if not creado:
            # Actualizar datos si el dispositivo ya existía
...
class TipoNotificacionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'codigo', 'nombre', 'descripcion', 'activo', 'requiere_autenticacion']
...
class NotificacionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Notificacion
        fields = [
            'id', 'titulo', 'mensaje', 'tipo_info', 'prioridad',
            'estado', 'fecha_creacion', 'fecha_envio', 'fecha_entrega', 'fecha_lectura',
            'data_extra', 'fue_leida', 'fue_entregada', 'usuario_username'
        ]
        read_only_fields = ['id', 'fecha_creacion', 'fecha_envio', 'fecha_entrega', 'fecha_lectura']
...
class NotificacionCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Notificacion
        fields = [
            'titulo', 'mensaje', 'tipo', 'prioridad', 'usuario', 'usuarios_ids',
            'data_extra', 'programada_para', 'expira_en'
        ]
    
    def validate(self, attrs):
        """Validar que se especifique al menos un destinatario"""
        if not attrs.get('usuario') and not attrs.get('usuarios_ids'):
            raise serializers.ValidationError("Debe especificar al menos un usuario o una lista de usuarios.")
        return attrs
...
class PreferenciaNotificacionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'nuevos_viajes', 'cambios_viaje', 'recordatorios', 'promociones', 'sistema',
            'no_molestar_desde', 'no_molestar_hasta',
            'push_mobile', 'push_web', 'email',
            'fecha_actualizacion'
        ]
