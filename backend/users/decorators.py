    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if not request.user.is_authenticated:
                return HttpResponseForbidden(_("Debes iniciar sesión para acceder a esta página"))
            
            # Verificar si el usuario tiene el permiso requerido
            if not request.user.tiene_permiso(permiso):
                return HttpResponseForbidden(_("No tienes permiso para acceder a esta funcionalidad"))
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if not request.user.tiene_permisos(permisos):
                return HttpResponseForbidden(_("No tienes los permisos necesarios para acceder a esta funcionalidad"))
...
def requiere_rol_administrativo(view_func):
    ...
    def _wrapped_view(request, *args, **kwargs):
        ...
        if not request.user.es_administrativo:
            return HttpResponseForbidden(_("Necesitas un rol administrativo para acceder a esta página"))
