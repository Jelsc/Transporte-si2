...
class EncomiendaService {
  ...
  Future<ApiResponse<Encomienda>> crearEncomienda(
    ...
    try {
      ...
      Logger.network(
        'Creando encomienda: ${request.toJson()}',
        tag: 'ENCOMIENDA',
      );
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        Logger.error(
          'Error al crear encomienda: $errorMessage',
          tag: 'ENCOMIENDA',
        );

        return ApiResponse<Encomienda>(success: false, error: errorMessage);
      }
    } catch (e) {
  ...
  Future<ApiResponse<List<Encomienda>>> getMisEncomiendas() async {
    try {
      ...
      Logger.network(
        'Mis encomiendas response: ${response.statusCode}',
        tag: 'ENCOMIENDA',
      );
      Logger.network(
        'Mis encomiendas body: ${response.body}',
        tag: 'ENCOMIENDA',
      );
      ...
      if (response.statusCode == 200) {
        ...
        Logger.debug(
          'Estructura de respuesta: ${responseData.runtimeType}',
          tag: 'ENCOMIENDA',
        );
        if (responseData is Map) {
          Logger.debug(
            'Keys del mapa: ${responseData.keys}',
            tag: 'ENCOMIENDA',
          );
        }
        ...
        Logger.success(
          'Encomiendas cargadas: ${encomiendas.length}',
          tag: 'ENCOMIENDA',
        );
        ...
        return ApiResponse<List<Encomienda>>(success: false, error: error);
      }
    } catch (e) {
  ...
  Future<ApiResponse<Encomienda>> getSeguimiento(String codigo) async {
    try {
      ...
      Logger.network(
        'Seguimiento response: ${response.statusCode}',
        tag: 'SEGUIMIENTO',
      );
      ...
      if (response.statusCode == 200) {
        ...
        return ApiResponse<Encomienda>(success: false, error: error);
      }
    } catch (e) {
  ...
  /// Geocodifica una dirección a coordenadas usando el backend
  Future<ApiResponse<Map<String, dynamic>>> geocodificarDireccion(
    String direccion,
  ) async {
    try {
      final baseUrl = await IPDetection.getBaseUrl();
      ...
      Logger.network('Geocodificando: $direccion', tag: 'GEOCODE');

      final response = await http
          .post(
            Uri.parse('$baseUrl/api/ubicaciones/geocode/'),
            headers: headers,
            body: json.encode({'direccion_texto': direccion}),
          )
      ...
      if (response.statusCode == 200) {
        ...
        Logger.success('Dirección geocodificada', tag: 'GEOCODE');

        return ApiResponse<Map<String, dynamic>>(
          success: true,
          data: responseData as Map<String, dynamic>,
          message: 'Dirección geocodificada correctamente',
        );
      } else if (response.statusCode == 404) {
        Logger.warning('Dirección no encontrada: $direccion', tag: 'GEOCODE');
        return ApiResponse<Map<String, dynamic>>(
          success: false,
        ...
        Logger.error('Error en geocodificación: $error', tag: 'GEOCODE');

        return ApiResponse<Map<String, dynamic>>(success: false, error: error);
      }
    } catch (e) {
      Logger.error('Error en geocodificarDireccion', tag: 'GEOCODE', error: e);
      return ApiResponse<Map<String, dynamic>>(
        success: false,
        error: 'Error de conexión: $e',
      );
  ...
  Future<ApiResponse<Encomienda>> actualizarEstado(
    int encomiendaId,
    String estado, {
    String? notas,
    String? ubicacion,
    ...
    try {
      ...
      Logger.network(
        'Actualizando estado de encomienda $encomiendaId a $estado',
        tag: 'ESTADO',
      );
  ...
  // ✅ MÉTODOS DE PAGO
  Future<ApiResponse<Map<String, dynamic>>> crearPagoStripe(
    int encomiendaId,
  ) async {
    try {
      final baseUrl = await IPDetection.getBaseUrl();
      ...
      Logger.network(
        'Creando pago Stripe para encomienda: $encomiendaId',
        tag: 'PAGO',
      );
  ...
  Future<ApiResponse<Encomienda>> confirmarPago(
    int encomiendaId,
    ...
    try {
      ...
      Logger.network(
        'Confirmando pago para encomienda: $encomiendaId',
        tag: 'PAGO',
      );
  ...
  Future<ApiResponse<Encomienda>> confirmarPagoStripe(
    int encomiendaId,
    String paymentIntentId,
  ) async {
    try {
      ...
      Logger.network(
        'Confirmando pago Stripe para encomienda: $encomiendaId',
        tag: 'PAGO_STRIPE',
      );
      ...
      Logger.network(
        'Confirmar pago response: ${response.statusCode}',
        tag: 'PAGO_STRIPE',
      );
      Logger.network(
        'Confirmar pago body: ${response.body}',
        tag: 'PAGO_STRIPE',
      );
      ...
      if (response.statusCode == 200) {
        ...
        Logger.success(
          'Pago Stripe confirmado exitosamente',
          tag: 'PAGO_STRIPE',
        );
        ...
        Logger.error(
          'Error al confirmar pago Stripe: $errorMessage',
          tag: 'PAGO_STRIPE',
        );
  ...
  Future<ApiResponse<Encomienda>> asignarViaje(
    int encomiendaId,
    int viajeId,
  ) async {
    try {
      ...
      Logger.network(
        'Asignando encomienda $encomiendaId al viaje $viajeId',
        tag: 'ASIGNAR_VIAJE',
      );

      final response = await http.post(
        Uri.parse('$baseUrl/api/encomiendas/$encomiendaId/asignar_viaje/'),
        headers: headers,
        body: json.encode({'viaje_id': viajeId}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        Logger.success(
          'Encomienda asignada al viaje exitosamente',
          tag: 'ASIGNAR_VIAJE',
        );
        ...
        return ApiResponse<Encomienda>(
          success: true,
          data: encomienda,
          message: 'Encomienda asignada al viaje exitosamente',
        );
      } else {
        final errorData = json.decode(response.body);
        final error = _parseError(errorData, response.statusCode);

        Logger.error('Error al asignar viaje: $error', tag: 'ASIGNAR_VIAJE');

        return ApiResponse<Encomienda>(success: false, error: error);
      }
    } catch (e) {
      Logger.error('Error en asignarViaje', tag: 'ASIGNAR_VIAJE', error: e);
      return ApiResponse<Encomienda>(
        success: false,
        error: 'Error de conexión: $e',
      );
  ...
  Future<ApiResponse<Encomienda>> marcarPagoEfectivo(int encomiendaId) async {
    try {
      ...
      Logger.network(
        'Marcando pago en efectivo para encomienda: $encomiendaId',
        tag: 'PAGO',
      );

      final response = await http.post(
        Uri.parse(
          '$baseUrl/api/encomiendas/$encomiendaId/marcar_pago_efectivo/',
        ),
        headers: headers,
      );

      Logger.network(
        'Pago efectivo response: ${response.statusCode}',
        tag: 'PAGO',
      );
